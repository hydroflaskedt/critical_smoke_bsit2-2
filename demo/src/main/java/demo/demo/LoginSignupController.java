...
public Map<String, Object> login (@RequestBody user request, HttpSession session, HttpServletResponse jar) {
    ...
    Boolean userFound = foundUser != null;
    ...
    Boolean isAdmin = foundUser.isAdmin();
...
public Map<String, Object> signup (@RequestBody user request) {
     user foundUser = repo.findByUsernameAndEmail(
        request.getUsername(),
        request.getEmail()
    );

    Boolean newUser = foundUser == null;
    ...
    if (newUser){
        repo.save(request);
        response.put("success", true);
        response.put("test", request.getUserPassword());
    }
    else {
